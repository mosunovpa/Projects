...
//	nBufLen   - allocated length of lpszBuffer in characters
...
BOOL CClipboard::GetText (LPTSTR lpszBuffer, int nBufLen, HWND hWnd)
{
	...
	hGlobal = GetClipboardData(TEXT_FORMAT);
...
int CClipboard::SetText (LPCTSTR lpszBuffer, HWND hWnd)
{
	...
	hGlobal = GlobalAlloc(GMEM_MOVEABLE, (nSize + 1) * sizeof(TCHAR));
	...
	memcpy(lpszData, lpszBuffer, (nSize + 1) * sizeof(TCHAR));
	...
	SetClipboardData(TEXT_FORMAT, hGlobal);
	CloseClipboard();
