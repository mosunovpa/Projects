...
#ifndef __SSFILEDIALOG_H__
#define __SSFILEDIALOG_H__

#include <atldlgs.h>

typedef struct tagOFNEXA {
   DWORD        lStructSize;
   HWND         hwndOwner;
   HINSTANCE    hInstance;
   LPCSTR       lpstrFilter;
   ...
   DWORD        dwReserved;
   DWORD        FlagsEx;
} OPENFILENAMEEXA, *LPOPENFILENAMEEXA;

typedef struct tagOFNEXW {
   DWORD        lStructSize;
   HWND         hwndOwner;
   HINSTANCE    hInstance;
   LPCWSTR      lpstrFilter;
   ...
   DWORD        dwReserved;
   DWORD        FlagsEx;
} OPENFILENAMEEXW, *LPOPENFILENAMEEXW;

#ifdef UNICODE
typedef OPENFILENAMEEXW OPENFILENAMEEX;
typedef LPOPENFILENAMEEXW LPOPENFILENAMEEX;
#else
typedef OPENFILENAMEEXA OPENFILENAMEEX;
...
public:
	...
	{ 
		...
		{
			const size_t len = _tcslen(lpszFilter) + 1;
			m_szFilter = new TCHAR[len];
			_tcscpy_s(m_szFilter, len, lpszFilter);
	...
	INT_PTR DoModal(HWND hWndParent = ::GetActiveWindow())
	{
		...
		{
			// Prepare m_ofnex:
			memcpy(&m_ofnex, &m_ofn, sizeof(OPENFILENAMEEX) - (sizeof(DWORD)*2) - sizeof(void *));
