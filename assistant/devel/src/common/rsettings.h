...
#define BEGIN_HKCU_REG_MAP(CLASS_NAME, KEY) \
	BEGIN_HKEY_REG_MAP(HKEY_CURRENT_USER, CLASS_NAME, KEY) \

#define BEGIN_HKEY_REG_MAP(ROOT_HKEY, CLASS_NAME, KEY) \
	CLASS_NAME() \
	{ \
		m_hKey = ROOT_HKEY; \
		m_sKey = KEY; \
...
#define BEGIN_INTERNAL_SAVE_LOAD() \
	virtual DWORD InternalSaveLoad(HKEY hKey, bool bSave, bool bOnlyInit) \
	{ \
		DWORD dwResult = ERROR_SUCCESS; \
		if(bOnlyInit) OnBeforeInit(); \
		if(bSave && !bOnlyInit) OnBeforeSave(); \

#define END_REG_MAP() \
	if(bOnlyInit) OnAfterInit(); \
	if(!bSave && !bOnlyInit) OnAfterLoad(); return dwResult; } \
...
#define REG_ITEM_SZ(variable, DEFAULT_VALUE) \
			if(bOnlyInit) _tcscpy_s(variable, sizeof(variable)/sizeof(TCHAR), DEFAULT_VALUE); else {\
			dwResult = SaveLoadItem(hKey, _T(#variable), variable, sizeof(variable)/sizeof(TCHAR), bSave); \
			if(dwResult != ERROR_SUCCESS) \
			if(bSave) return dwResult; else _tcscpy_s(variable, sizeof(variable)/sizeof(TCHAR), DEFAULT_VALUE); }
